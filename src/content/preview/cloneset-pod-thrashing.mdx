...
CloneSet rolling updates are governed by two parameters: `maxUnavailable` (how many pods can be down at once) and `maxSurge` (how many extra pods to create above the desired count). Together they control the rollout pace. Step through a 100-replica rollout with maxSurge at 10% and maxUnavailable at 5%:
