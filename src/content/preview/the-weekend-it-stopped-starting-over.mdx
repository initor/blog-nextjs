...
Over three days, four of us shipped ten production-ready agent skills across a workload platform end-to-end. One agent, ten structured specifications, zero hand-written implementation code. The speed was real, but it was not the deepest part of the story.

The real discovery came after the weekend. The agents could build, yet they could not continue. End a session and everything the agent had learned vanished with it. The scarce resource was not intelligence or execution. It was the architecture for persisting state so work could compound instead of restart.

Since January, that insight has started rearranging a workflow I had shaped over the past decade.
...
I did not invent the answer on my own. Once I understood the problem, I opened a new ChatGPT project and asked a narrower question: if the context window is ephemeral, what should persist outside it? The answer was not a better paragraph. It was a small set of files written for the next session.
...
The agent can carry out work. The harder problem is guiding it toward work I would be proud to ship. That means encoding taste: architectural preferences, quality bars, invariants, trade-offs, and what not to do. It also means learning taste faster ourselves, because the leverage has moved up a layer.
...
## Execution Gets Cheap. Taste Does Not.

For most of my career, the bottleneck was knowledge. You learned by reading papers, tracing source code, and accumulating repetitions over years. Then chatbots collapsed the cost of asking. Learning got faster, but execution was still mostly yours.
